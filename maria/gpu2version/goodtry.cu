...
matmultgpu2(int m, int n, int k, double *A, double *B, double *C) {
 ...
 float Cvalue=0;
  ...
  int e,gg;
  for(e=0;e<(blockDim.x+k-1)/blockDim.x;++e){
     for(gg=0;gg<blockDim.x;gg++){
        if((e*blockDim.x+gg<k&&row<m)&&(e*blockDim.x+gg<k&&col<n))
           Cvalue+=A[row*k+e*blockDim.x+gg]*B[(e*blockDim.x+gg)*n+col];
}
