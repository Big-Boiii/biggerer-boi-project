...
int main(int argc, char *argv[]){
    ...
    double  ***u_d0, ***u_d1=NULL;
	double  ***f_d0, ***f_d1=NULL;
	double  ***prev_u_d0, ***prev_u_d1=NULL;
    ...
    if ( (u_d0 = d_malloc_3d_gpu(N, N, N)) == NULL ) {
        perror("array u: allocation failed");
        exit(-1);
    }
	
    if ( (f_d0 = d_malloc_3d_gpu(N,N,N)) == NULL ) {
		perror("array f: allocation failed");
		exit(-1);
    }
    if ( (prev_u_d0 = d_malloc_3d_gpu(N, N, N)) == NULL ) {
		perror("array f: allocation failed");
		exit(-1);
    }  
    ...
    if ( (u_d1 = d_malloc_3d_gpu(N, N, N)) == NULL ) {
        perror("array u: allocation failed");
        exit(-1);
    }
	
    if ( (f_d1 = d_malloc_3d_gpu(N,N,N)) == NULL ) {
		perror("array f: allocation failed");
		exit(-1);
    }
    if ( (prev_u_d1 = d_malloc_3d_gpu(N, N, N)) == NULL ) {
		perror("array f: allocation failed");
		exit(-1);
    }   
    ...
    //copy assigned Matrices to GPU 0
    cudaSetDevice(0);
    transfer_3d(u_d0, u_h, N,N,N, cudaMemcpyHostToDevice);
    transfer_3d(prev_u_d0, prev_u_h, N,N,N, cudaMemcpyHostToDevice);
    transfer_3d(f_d0,f_h, N,N,N, cudaMemcpyHostToDevice);
    
    //copy assigned Matrices to GPU 1
    cudaSetDevice(1);
    transfer_3d(u_d1, u_h, N,N,N, cudaMemcpyHostToDevice);
    transfer_3d(prev_u_d1, prev_u_h, N,N,N, cudaMemcpyHostToDevice);
    transfer_3d(f_d1,f_h, N,N,N, cudaMemcpyHostToDevice);
    ...
    dim3 grid_size((N-2)*0.5/block_size.x, (N-2)/ block_size.y, (N-2)/block_size.z);
    ...
    for (i=0; i< iter_max; i++){
		...
		checkCudaErrors(cudaSetDevice(0));
		jacobi_gpu2<<<grid_size,block_size>>>(u_d0,prev_u_d0,f_d0,N, step_width, denominator, 0);	
